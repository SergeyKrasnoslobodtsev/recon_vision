...
class ReconServiceClient:
    ...
    def send_pdf(self, pdf_path: str) -> str:
        ...
        # Отправляем запрос
        response = self.session.post(f"{self.base_url}/send_reconciliation_act", json={"document": pdf_b64})
    ...
    def wait_for_processing(self, process_id: str, timeout: int = 10000) -> Dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            response = self.session.post(f"{self.base_url}/process_status", json={"process_id": process_id})
    ...
    def fill_and_get_pdf(
        self, process_id: str, debit_entries: list, credit_entries: list, output_path: str = None
    ) -> str:
        ...
        response = self.session.post(f"{self.base_url}/fill_reconciliation_act", json=fill_request)
    ...
    def _print_extraction_results(self, result: Dict[str, Any]):
        ...
        if debit:
            ...
            for i, entry in enumerate(debit[:3]):
                print(
                    f"      {i + 1}. Таблица {entry['row_id']['id_table']}, строка {entry['row_id']['id_row']}: {entry['value']}"
                )
...
def modify_values_example(pdf_path: str, server_url: str = "http://127.0.0.1:8000"):
    ...
    try:
        ...
        output_path = client.fill_and_get_pdf(
            process_id, debit_entries, credit_entries, f"modified_document_{process_id[:8]}.pdf"
        )
