...
class TestDebugImageObserver:
    ...
    def test_saves_canonical_document_overlay_for_real_pdf(self, pdf_file: Path, output_dir: Path):
        ...
        logger.info(f"Визуализация канонического документа: {pdf_file.name}")
        ...
        with PDFLoader(pdf_bytes) as loader:
            for page in document.pages:
                ...
                observer.on_canonical_page(source_image, page=page, stage="canonical_document", prefix=pdf_file.stem)

        first_page_path = output_dir / "canonical_document" / f"{pdf_file.stem}_000.png"
